...
class AgentService:
  ...
  async def run(self, prompt: str, guild: Optional[discord.Guild] = None, context: str = "") -> str:
    ...
    try:
      result = await Runner.run(self.agent, full_prompt, context=BotContext(guild=guild))
